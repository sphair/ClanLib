...
To connect to a database, you use the \ref clan::DBConnection class. You need to use a provider specific implementation of it, since it depends on the target database system how to connect to it. ClanLib provides one provider currently, the Sqlite provider. Others might be added later, for instance MySQL or MS SQL Server. 
<br/>
...
To issue a SQL query to a connection, you use the \ref clan::DBCommand class. It is created using create_command on the connection object, and it takes a query string and a type. Type can be a sql statement (default) or a stored procedure. Sqlite only supports sql statements, but future providers might support both. 
<br/>
\code
...
clan::DBCommand command = connection.create_command("SELECT UserId, UserName, CreateDate FROM Users WHERE UserId=?1");
command.set_input_parameter_int(1, user_id);
clan::DBReader reader = connection.execute_reader(command);
while (reader.retrieve_row())
{
	int userId = reader.get_column_int(0);
	clan::String user_name = reader.get_column_string(1);
	clan::DateTime create_date = reader.get_column_datetime(2);
}
reader.close();
\endcode
...
The ClanLib database API is modelled after a provider pattern, making it is possible to add support for new database systems. If you want to look into this, you basically have to make provider-specific implementations of \ref clan::DBConnection, \ref clan::DBCommandProvider, \ref clan::DBConnectionProvider, \ref clan::DBReaderProvider and \ref clan::DBTransactionProvider. See the ClanSqlite implementation for an example of this. 
