...
void linear3(Scanline *d)
{
	...
	for (int x = d->startx; x < d->endx; x+=2)
	{
		...
		srcpixels[0] = sampleLinearRepeat3(tx, ty, texwidth, texheight, texdata); tx += dtx; ty += dty;
		srcpixels[1] = sampleLinearRepeat3(tx, ty, texwidth, texheight, texdata); tx += dtx; ty += dty;
