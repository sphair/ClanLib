...
void nearest1(Scanline *d)
{
	...
	for (int x = d->startx; x < d->endx; x+=2)
	{
		...
		srcpixels[0] = sampleNearestRepeat(tx, ty, texwidth, texheight, texdata); tx += dtx; ty += dty;
		srcpixels[1] = sampleNearestRepeat(tx, ty, texwidth, texheight, texdata); tx += dtx; ty += dty;
