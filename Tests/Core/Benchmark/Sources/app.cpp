...
int App::start(const std::vector<std::string> &args)
{
	...
	while (!quit)
	{
		...
		{
			font.draw_text(canvas, 10, ypos, clan::string_format("%1 = Base Line Test", result_empty));
			ypos += ygap;
		}
...
void App::initialise_1()
{
		...
		for (num_iterations = 0; ; num_iterations++)
		{
			...
			if ((current_time - start_time) >= (1000000) * 4)		// Run each test for max 4 seconds
				break;
...
void App::initialise_2()
{
		...
		clan::ubyte64 current_time = clan::System::get_microseconds();
		base_line = current_time - start_time;
