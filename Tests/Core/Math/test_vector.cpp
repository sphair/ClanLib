...
void TestApp::test_vector2(void)
{
	...
	Console::write_line("   Function: rotate()");
	{
		...
		test_a = Vec2i(4,5);
		test_a.rotate(hotspot, Angle(0, cl_degrees));
		if (test_a != Vec2i(4, 5))  fail();

		test_a = Vec2i(4,5);
		test_a.rotate(hotspot, Angle(90, cl_degrees));
		if (test_a != Vec2i(-1, 6))  fail();

		test_a = Vec2i(4,5);
		test_a.rotate(hotspot, Angle(180, cl_degrees));
		if (test_a != Vec2i(-2, 1))  fail();

		test_a = Vec2i(4,5);
		test_a.rotate(hotspot, Angle(270, cl_degrees));
		if (test_a != Vec2i(3, 0))  fail();

		test_a = Vec2i(4,5);
		test_a.rotate(hotspot, Angle(360, cl_degrees));
		if (test_a != Vec2i(4, 5))  fail();

		test_a = Vec2i(4,5);
		test_a.rotate(Vec2i(0,0), Angle(180, cl_degrees));
		if (test_a != Vec2i(-4, -5))  fail();
...
{
	Console::write_line(" Header: cl_vector.h");
	Console::write_line("  Class: Vec1");


	Console::write_line("   Function: operator += (const Vec1<Type>& vector)");
	{
		Vec1d testd(2.5);
		testd += Vec1d(1.0);
	...
	{
		Vec1d testd(2.5);
		if (testd == Vec1d(1.0)) fail();
		if (!(testd == Vec1d(2.5))) fail();

		Vec1i testi(2);
		if (testi == Vec1i(1)) fail();
		if (!(testi == Vec1i(2))) fail();
	}
	...
	{
		Vec1d testd(2.0);
		Vec1d dest = Vec1d::round(testd);

		if (dest.x != 2.0) fail();

		Vec1f testf(2.0f);
		Vec1f dest2 = Vec1f::round(testf);
