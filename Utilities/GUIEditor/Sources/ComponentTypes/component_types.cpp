...
void ComponentTypes::set_id_name(GUIComponent *new_component, int id)
{
	if (component_count.find(id) == component_count.end())
		component_count[id] = 1;

	new_component->set_id_name(string_format("%1%2", new_component->get_type_name(), component_count[id]++));
}
