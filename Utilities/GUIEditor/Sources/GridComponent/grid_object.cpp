...
DomElement GridObject::to_element(DomDocument &doc)
{
	...
	DomElement e = doc.create_element(comp->get_type_name());
...
void GridObject::on_render(GraphicContext &gc, const Rect &update_rect)
{
	// Do a fill rect for otherwise transparent components:
	std::string type = get_first_child()->get_type_name();
	if (type == "toolbar" || type == "menubar" /*|| type == "label"*/)
	{
		Rect child_geom = get_first_child()->get_geometry();
		Draw::fill(get_gc(), child_geom, Colorf(0.35f, 0.498f, 0.603f, 0.2f));
	}
	...
	gc.set_font(font);
	gc.draw_text(20,24, string_format("%1, %2", tl.x, tl.y), Colorf::red);
	gc.draw_text(60,24, string_format("%1, %2", tl_win.x, tl_win.y), Colorf::blue);
	*/
 }
