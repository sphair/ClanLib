...
void PropertyComponent::on_selection_changed()
{
	...
	if (!selection.empty())
	{
		...
		std::string type = comp->get_type_name();
...
void PropertyComponent::on_render(GraphicContext &gc, const Rect &update_rect)
{
	update_layout();
...
void PropertyComponent::on_filter_message(GUIMessage &message)
{
	if (active_component)
	{
		if (message.get_type() == GUIMessage_Input::get_type_name())
		{
			...
			InputEvent e = input_message.get_event();
			if (e.type == InputEvent::pressed && e.device.get_type() == InputDevice::pointer)
			{
...
void PropertyComponent::update_layout()
{
	...
	header = Rect(content.left, content.top, content.right, content.top+part_header.get_preferred_height());
...
void PropertyComponent::update_scrollbar_size(GraphicContext &gc)
{
	int row_width = content.get_width(); // to do: account for shrink box
