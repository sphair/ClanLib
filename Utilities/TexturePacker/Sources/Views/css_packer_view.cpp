...
CssPackerView::CssPackerView(GUIComponent *parent, MainWindow *mainwindow)
: View(parent, mainwindow, "CSS Packer")
{
	func_resized().set(this, &CssPackerView::on_resized);

	label_selected_file = new Label(this);
	label_selected_file->set_text("Input CSS file:");

	lineedit_selected_file = new LineEdit(this);

	button_browse_file = new PushButton(this);
...
void CssPackerView::on_resized()
{
	...
	label_selected_file->set_geometry(Rect(10, 13, 80, 30));
	lineedit_selected_file->set_geometry(Rect(86, 10, size.width - 20 - 20, 30));
	button_browse_file->set_geometry(Rect(10 + size.width - 20 - 30, 10, 10 + size.width - 20, 30));

	button_save->set_geometry(Rect(size.width - 130, 70, Size(120, 25)));

	label_result->set_geometry(Rect(10, 100, size.width - 20, 130));
}
...
void CssPackerView::on_button_browse()
{
	...
	if(dlg.show())
	{
		...
		try
		{
			...
			label_result->set_text(string_format("Loaded CSS file with %1 imports", imports));
...
void CssPackerView::on_button_save()
{
	...
	if(dlg.show())
	{
		...
		try
		{
			...
			label_result->set_text(string_format("Saved resources: %1", filename));
		}
		catch(Exception error)
		{
