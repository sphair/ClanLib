...
CSSTokenizer_Impl::CSSTokenizer_Impl(IODevice &device)
: pos(0)
{
	DataBuffer buffer(device.get_size());
	device.receive(buffer.get_data(), buffer.get_size(), true);
	doc = StringHelp::utf8_to_text(std::string(buffer.get_data(), buffer.get_size()));
}

CSSTokenizer_Impl::CSSTokenizer_Impl(const std::string &text)
: doc(text), pos(0)
{
}

void CSSTokenizer_Impl::peek(CSSToken &out_token)
{
	size_t cur_pos = pos;
	read(out_token);
	pos = cur_pos;
...
void CSSTokenizer_Impl::read(CSSToken &token)
{
	token.type = CSSToken::type_null;
	...
	case ':':
		...
		break;
	case ';':
		token.type = CSSToken::type_semi_colon;
		pos++;
		break;
	case '{':
		token.type = CSSToken::type_curly_brace_begin;
		pos++;
		break;
	case '}':
		token.type = CSSToken::type_curly_brace_end;
		pos++;
		break;
	case '(':
		token.type = CSSToken::type_bracket_begin;
		pos++;
		break;
	case ')':
		token.type = CSSToken::type_bracket_end;
		pos++;
		break;
	case '[':
		...
		break;
	case ']':
		token.type = CSSToken::type_square_bracket_end;
		pos++;
		break;
	case '/':
		read_comment(token);
	...
	{
		token.type = CSSToken::type_ident;
		return;
	}
	...
	if (doc[pos] == '"')
	{
		...
		if (end_pos != pos+1)
		{
			token.type = CSSToken::type_string;
			pos = end_pos;
			return;
		}
		else
		{
			token.type = CSSToken::type_invalid;
	...
	{
		token.type = CSSToken::type_delim;
		token.value.append(1, doc[pos]);
		pos++;
	}
}

void CSSTokenizer_Impl::read_number_type(CSSToken &token)
{
	...
	if (!(pos+1 == end_pos && dot_encountered))
	{
		...
		{
			token.type = CSSToken::type_percentage;
			pos = end_pos+1;
		}
		else
		{
			...
			if (!token.dimension.empty())
				token.type = CSSToken::type_dimension;
			else
				token.type = CSSToken::type_number;
			pos = end_pos;
		}
	}
}

void CSSTokenizer_Impl::read_atkeyword(CSSToken &token)
{
	if (pos+2 <= doc.length() && doc[pos] == '@')
	{
		pos = read_ident(pos+1, token.value);
		if (!token.value.empty())
			token.type = CSSToken::type_atkeyword;
	}
}

void CSSTokenizer_Impl::read_hash(CSSToken &token)
{
	if (pos+2 <= doc.length() && doc[pos] == '#')
	{
...
size_t CSSTokenizer_Impl::read_ident(size_t p, std::string &out_ident)
{
	out_ident.clear();
...
size_t CSSTokenizer_Impl::read_name(size_t p, std::string &out_ident)
{
	out_ident.clear();
...
void CSSTokenizer_Impl::read_cdo(CSSToken &token)
{
	if (pos+4 <= doc.length() && doc[pos] == '<' && doc[pos+1] == '!' && doc[pos+2] == '-' && doc[pos+3] == '-')
	{
		token.type = CSSToken::type_cdo;
		token.value.clear();
		pos += 4;
	}
}

void CSSTokenizer_Impl::read_cdc(CSSToken &token)
{
	if (pos+3 <= doc.length() && doc[pos] == '-' && doc[pos+1] == '-' && doc[pos+2] == '>')
	{
		token.type = CSSToken::type_cdc;
		token.value.clear();
		pos += 3;
	}
}

void CSSTokenizer_Impl::read_comment(CSSToken &token)
{
	if (pos+4 <= doc.length() && doc[pos] == '/' && doc[pos+1] == '*')
	{
		...
		{
			token.type = CSSToken::type_comment;
			pos = end_pos+1;
		}
	}
}

void CSSTokenizer_Impl::read_uri(CSSToken &token)
{
	if (pos+5 <= doc.length() &&
		(doc[pos] == 'u' || doc[pos] == 'U') &&
		...
		doc[pos+3] == '(')
	{
		...
		if (end_pos < doc.length() && doc[end_pos] == ')')
		{
			token.type = CSSToken::type_uri;
			pos = end_pos+1;
		}
	}
}

void CSSTokenizer_Impl::read_function(CSSToken &token)
{
	...
	if (end_pos != pos)
	{
		...
		{
			pos = end_pos+1;
			token.type = CSSToken::type_function;
		}
	}
}

void CSSTokenizer_Impl::read_whitespace(CSSToken &token)
{
...
void CSSTokenizer_Impl::read_includes(CSSToken &token)
{
	if (pos+2 <= doc.length() && doc[pos] == '~' && doc[pos+1] == '=')
	{
		token.type = CSSToken::type_includes;
		token.value.clear();
		pos += 2;
	}
}

void CSSTokenizer_Impl::read_dashmatch(CSSToken &token)
{
	if (pos+2 <= doc.length() && doc[pos] == '|' && doc[pos+1] == '=')
	{
		token.type = CSSToken::type_dashmatch;
		token.value.clear();
		pos += 2;
	}
}

size_t CSSTokenizer_Impl::read_nmstart(size_t p, std::string::value_type &out_c)
{
	out_c = '?';
...
size_t CSSTokenizer_Impl::read_nmchar(size_t p, std::string::value_type &out_c)
{
	if (p+1 <= doc.length())
	{
		out_c = doc[p];
...
bool CSSTokenizer_Impl::is_whitespace(std::string::value_type c)
{
	switch (c)
	{
	case ' ':
...
size_t CSSTokenizer_Impl::read_string(size_t p, std::string &out_str, std::string::value_type str_char) const
{
	out_str.clear();
	size_t end_pos = p;
...
size_t CSSTokenizer_Impl::read_invalid(size_t p) const
{
	size_t end_pos = p;
...
size_t CSSTokenizer_Impl::read_uri_nonquoted_string(size_t p, std::string &out_str) const
{
	out_str.clear();
	size_t end_pos = p;
