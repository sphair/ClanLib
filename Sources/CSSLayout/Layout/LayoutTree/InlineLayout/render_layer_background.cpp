...
bool CSSInlineLayoutRenderLayerBackground::node(CSSInlineGeneratedBox *cur)
{
	...
	else if (element)
	{
		if (element->computed_properties.visibility.type == CSSValueVisibility::type_visible)
		{
			...
			padding_box.expand(
				cur->opening ? used_to_actual(tree_node->get_css_padding_width(element->computed_properties.padding_width_left, tree_node->containing_width)) : 0,
				used_to_actual(tree_node->get_css_padding_height(element->computed_properties.padding_width_top, tree_node->containing_height)),
				cur->closing ?  used_to_actual(tree_node->get_css_padding_width(element->computed_properties.padding_width_right, tree_node->containing_width)) : 0,
				used_to_actual(tree_node->get_css_padding_height(element->computed_properties.padding_width_bottom, tree_node->containing_height)));

			Rect border_box = padding_box;
			border_box.expand(
				cur->opening ? used_to_actual(element->computed_properties.border_width_left.length.value) : 0,
				used_to_actual(element->computed_properties.border_width_top.length.value),
