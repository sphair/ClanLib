...
void CSSParserBottom::parse(CSSBoxProperties &properties, const std::string &name, const std::vector<CSSToken> &tokens, std::map<std::string, CSSBoxProperty *> *out_change_set)
{
	size_t pos = 0;
	...
	if (token.type == CSSToken::type_ident && pos == tokens.size())
	{
		if (equals(token.value, "auto"))
			...
			properties.bottom.type = CSSBoxBottom::type_inherit;
	}
	else if (is_length(token) && pos == tokens.size())
	{
		...
		{
			properties.bottom.type = CSSBoxBottom::type_length;
			properties.bottom.length = length;
		}
	}
	else if (token.type == CSSToken::type_percentage && pos == tokens.size())
	{
		properties.bottom.type = CSSBoxBottom::type_percentage;
