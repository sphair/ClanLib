...
void CSSParserLeft::parse(CSSBoxProperties &properties, const std::string &name, const std::vector<CSSToken> &tokens, std::map<std::string, CSSBoxProperty *> *out_change_set)
{
	size_t pos = 0;
	...
	if (token.type == CSSToken::type_ident && pos == tokens.size())
	{
		if (equals(token.value, "auto"))
			...
			properties.left.type = CSSBoxLeft::type_inherit;
	}
	...
	{
		properties.left.type = CSSBoxLeft::type_percentage;
