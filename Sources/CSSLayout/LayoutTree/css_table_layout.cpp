...
void CSSTableLayout::layout_content(CSSLayoutGraphics *graphics, CSSLayoutCursor &cursor, LayoutStrategy strategy)
{
	...
	for (size_t i = 0; i < captions.size(); i++)
	{
		if (captions[i]->get_element_node()->computed_properties.caption_side.type == CSSBoxCaptionSide::type_top)
		{
	...
	for (size_t i = 0; i < captions.size(); i++)
	{
		if (captions[i]->get_element_node()->computed_properties.caption_side.type == CSSBoxCaptionSide::type_bottom)
		{
			captions[i]->containing_height = height;
			captions[i]->calculate_content_top_down_heights();
			captions[i]->containing_width = width;
...
Rect CSSTableLayout::get_cell_border_box(size_t row, size_t col)
{
	...
	if (element_node->computed_properties.border_collapse.type == CSSBoxBorderCollapse::type_collapse)
	{
...
void CSSTableLayout::render_cell_non_content(CSSLayoutGraphics *graphics, CSSResourceCache *resources, size_t row, size_t col)
{
	...
	if (cell->get_element_node()->computed_properties.border_style_top.type == CSSBoxBorderStyle::type_solid)
	...
	if (cell->get_element_node()->computed_properties.border_style_bottom.type == CSSBoxBorderStyle::type_solid)
	...
	if (cell->get_element_node()->computed_properties.border_style_left.type == CSSBoxBorderStyle::type_solid)
	...
	if (cell->get_element_node()->computed_properties.border_style_right.type == CSSBoxBorderStyle::type_solid)
	...
	for (size_t row = 0; row < rows.size(); row++)
	{
		for (size_t cell = 0; cell < columns.size(); cell++)
		{
			...
			if (get_layout(cell, row))
			{
				...
				if (element_node->computed_properties.border_collapse.type == CSSBoxBorderCollapse::type_separate)
...
void CSSTableLayout::calculate_cell_widths(CSSLayoutGraphics *graphics, CSSLayoutCursor & cursor, LayoutStrategy strategy)
{
	...
	if (element_node->computed_properties.width.type == CSSBoxWidth::type_auto)
	{
	...
	for (size_t row = 0; row < rows.size(); row++)
	{
		...
		for (size_t cell = 0; cell < columns.size(); cell++)
		{
			...
			if (get_layout(cell, row))
			{
				...
				if (get_layout(cell, row)->get_element_node()->computed_properties.vertical_align.type == CSSBoxVerticalAlign::type_middle)
				{
...
void CSSTableLayout::render_layer_non_inline(CSSLayoutGraphics *graphics, CSSResourceCache *resources)
{
	for (size_t i = 0; i < captions.size(); i++)
	{
		...
		bool is_positioned = (captions[i]->get_element_node()->computed_properties.position.type != CSSBoxPosition::type_static);
	...
	for (size_t row = 0; row < rows.size(); row++)
	{
		for (size_t cell = 0; cell < columns.size(); cell++)
		{
			if (get_layout(cell, row))
			{
				...
				bool is_positioned = (cell_node->get_element_node()->computed_properties.position.type != CSSBoxPosition::type_static);
