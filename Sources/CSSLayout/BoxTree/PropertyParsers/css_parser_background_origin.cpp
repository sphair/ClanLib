...
std::vector<CL_String> CL_CSSParserBackgroundOrigin::get_names()
{
...
void CL_CSSParserBackgroundOrigin::parse(CL_CSSBoxProperties &properties, const CL_String &name, const std::vector<CL_CSSToken> &tokens, std::map<CL_String, CL_CSSBoxProperty *> *out_change_set)
{
	size_t pos = 0;
	...
	else
	{
		...
		background_origin.type = CL_CSSBoxBackgroundOrigin::type_value;
		...
		while (true)
		{
			...
			{
				background_origin.values.push_back(CL_CSSBoxBackgroundOrigin::origin_border_box);
			}
			else if (equals(token.value, "padding-box"))
			{
				background_origin.values.push_back(CL_CSSBoxBackgroundOrigin::origin_padding_box);
			}
			else if (equals(token.value, "content-box"))
			{
				background_origin.values.push_back(CL_CSSBoxBackgroundOrigin::origin_content_box);
			}
			else
			{
				return;
