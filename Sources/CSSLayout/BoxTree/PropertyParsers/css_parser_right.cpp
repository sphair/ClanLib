...
std::vector<CL_String> CL_CSSParserRight::get_names()
{
...
void CL_CSSParserRight::parse(CL_CSSBoxProperties &properties, const CL_String &name, const std::vector<CL_CSSToken> &tokens, std::map<CL_String, CL_CSSBoxProperty *> *out_change_set)
{
	size_t pos = 0;
	...
	if (token.type == CL_CSSToken::type_ident && pos == tokens.size())
	{
		if (equals(token.value, "auto"))
			properties.right.type = CL_CSSBoxRight::type_auto;
		else if (equals(token.value, "inherit"))
			properties.right.type = CL_CSSBoxRight::type_inherit;
	}
