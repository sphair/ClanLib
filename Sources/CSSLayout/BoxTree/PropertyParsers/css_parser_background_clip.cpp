...
std::vector<CL_String> CL_CSSParserBackgroundClip::get_names()
{
...
void CL_CSSParserBackgroundClip::parse(CL_CSSBoxProperties &properties, const CL_String &name, const std::vector<CL_CSSToken> &tokens, std::map<CL_String, CL_CSSBoxProperty *> *out_change_set)
{
	size_t pos = 0;
	...
	else
	{
		...
		background_clip.type = CL_CSSBoxBackgroundClip::type_value;
		...
		while (true)
		{
			...
			{
				background_clip.values.push_back(CL_CSSBoxBackgroundClip::clip_border_box);
			}
			else if (equals(token.value, "padding-box"))
			{
				background_clip.values.push_back(CL_CSSBoxBackgroundClip::clip_padding_box);
			}
			else if (equals(token.value, "content-box"))
			{
				background_clip.values.push_back(CL_CSSBoxBackgroundClip::clip_content_box);
			}
			else
			{
				return;
