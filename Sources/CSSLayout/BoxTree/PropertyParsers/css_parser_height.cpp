...
std::vector<CL_String> CL_CSSParserHeight::get_names()
{
...
void CL_CSSParserHeight::parse(CL_CSSBoxProperties &properties, const CL_String &name, const std::vector<CL_CSSToken> &tokens, std::map<CL_String, CL_CSSBoxProperty *> *out_change_set)
{
	size_t pos = 0;
	...
	else if (is_length(token) && pos == tokens.size())
	{
		...
		{
			properties.height.type = CL_CSSBoxHeight::type_length;
	...
	else if (token.type == CL_CSSToken::type_percentage && pos == tokens.size())
	{
		...
		if (v >= 0.0f)
		{
			properties.height.type = CL_CSSBoxHeight::type_percentage;
			properties.height.percentage = v;
		}
	}
	if (out_change_set)
	{
		(*out_change_set)["height"] = &properties.height;
	}
}
