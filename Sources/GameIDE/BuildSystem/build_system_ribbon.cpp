	...
	: UIControllerListener(controller)
{
	...
	button_build->func_clicked() = bind_member(this, &BuildSystemRibbon::on_button_build);

	button_rebuild = new PushButton(build_group1);
	button_rebuild->set_text("Rebuild");
	button_rebuild->func_clicked() = bind_member(this, &BuildSystemRibbon::on_button_build);

	button_clean = new PushButton(build_group1);
	button_clean->set_text("Clean");
	button_clean->func_clicked() = bind_member(this, &BuildSystemRibbon::on_button_clean);

	button_show_output = new PushButton(build_group2);
	button_show_output->set_text("Output");
	button_show_output->func_clicked() = bind_member(this, &BuildSystemRibbon::on_button_show_output);
