...
/////////////////////////////////////////////////////////////////////////////
// CL_InputDeviceProvider_SDLKeyboard construction:

CL_InputDeviceProvider_SDLKeyboard::CL_InputDeviceProvider_SDLKeyboard(CL_SDLWindow *window)
: sig_provider_event(0), window(window)
{
}

CL_InputDeviceProvider_SDLKeyboard::~CL_InputDeviceProvider_SDLKeyboard()
{
}

/////////////////////////////////////////////////////////////////////////////
// CL_InputDeviceProvider_SDLKeyboard attributes:

bool CL_InputDeviceProvider_SDLKeyboard::get_keycode(int keycode) const
{
	// Ignore all key events when we don't have focus
	if (!window->has_focus())
		return false;

	state_const_iterator pos = state.find(keycode);
	return (pos != state.end());
}

CL_String CL_InputDeviceProvider_SDLKeyboard::get_key_name(int virtual_key) const
{
	...
	return cl_format("Unknown %1", virtual_key);
}

float CL_InputDeviceProvider_SDLKeyboard::get_axis(int index) const
{
	return 0.0f;
}

CL_String CL_InputDeviceProvider_SDLKeyboard::get_name() const
{
	return cl_text("System Keyboard");
}

CL_String CL_InputDeviceProvider_SDLKeyboard::get_device_name() const
{
	return cl_text("System Keyboard");
}

int CL_InputDeviceProvider_SDLKeyboard::get_axis_count() const
{
	return 0;
}

int CL_InputDeviceProvider_SDLKeyboard::get_button_count() const
{
	return -1;
}

/////////////////////////////////////////////////////////////////////////////
// CL_InputDeviceProvider_SDLKeyboard operations:
void CL_InputDeviceProvider_SDLKeyboard::handle_keyboard_event(SDL_KeyboardEvent& event)
{
	static std::map<int, int> sdl_to_clanlib_keys;
	static bool first_call = true;
	...
	cl_event.id = sdl_to_clanlib_keys[event.keysym.sym];
	...
	cl_event.mouse_pos = CL_Point(window->mouse->get_x(), window->mouse->get_y());

	int mod_state = SDL_GetModState();

	cl_event.repeat_count = 0;
	cl_event.shift = (mod_state & KMOD_SHIFT) != 0;
	cl_event.alt = (mod_state & KMOD_ALT) != 0;
	cl_event.ctrl = (mod_state & KMOD_CTRL) != 0;

	wchar_t buff[2];
	buff[0] = event.keysym.unicode;
	buff[1] = 0;
	cl_event.str = CL_StringHelp::ucs2_to_text(CL_String16(buff));
	
	sig_provider_event->invoke(cl_event);
}

/////////////////////////////////////////////////////////////////////////////
// CL_InputDeviceProvider_SDLKeyboard implementation:
