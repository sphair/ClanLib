...
{
	cc.connect(display_window.sig_window_close(), Callback<void()>(this, &GUIWindowManagerProvider_Direct::on_displaywindow_window_close));

	InputContext ic = display_window.get_ic();
    cc.connect(ic.get_mouse().sig_key_up(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_up));
    cc.connect(ic.get_mouse().sig_key_down(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_down));
    cc.connect(ic.get_mouse().sig_key_dblclk(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_down));
    cc.connect(ic.get_mouse().sig_pointer_move(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_move));

    cc.connect(ic.get_keyboard().sig_key_up(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input));
    cc.connect(ic.get_keyboard().sig_key_down(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input));

	for (int tc = 0; tc < ic.get_tablet_count(); ++tc)
	{
        cc.connect(ic.get_tablet(tc).sig_axis_move(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_move));
        cc.connect(ic.get_tablet(tc).sig_key_dblclk(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_down));
        cc.connect(ic.get_tablet(tc).sig_key_down(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input_mouse_down));
        cc.connect(ic.get_tablet(tc).sig_key_up(), Callback<void(const InputEvent&)>(this, &GUIWindowManagerProvider_Direct::on_input));
	}
