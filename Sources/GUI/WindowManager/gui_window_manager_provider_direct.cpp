...
{
	display_window.sig_window_close().connect({this, &GUIWindowManagerProvider_Direct::on_displaywindow_window_close});

	InputContext ic = display_window.get_ic();
	ic.get_mouse().sig_key_up().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_up});
	ic.get_mouse().sig_key_down().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_down});
    ic.get_mouse().sig_key_dblclk().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_down});
    ic.get_mouse().sig_pointer_move().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_move});

    ic.get_keyboard().sig_key_up().connect({this, &GUIWindowManagerProvider_Direct::on_input});
    ic.get_keyboard().sig_key_down().connect({this, &GUIWindowManagerProvider_Direct::on_input});

	for (int tc = 0; tc < ic.get_tablet_count(); ++tc)
	{
        ic.get_tablet(tc).sig_axis_move().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_move});
        ic.get_tablet(tc).sig_key_dblclk().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_down});
        ic.get_tablet(tc).sig_key_down().connect({this, &GUIWindowManagerProvider_Direct::on_input_mouse_down});
        ic.get_tablet(tc).sig_key_up().connect({this, &GUIWindowManagerProvider_Direct::on_input});
	}
