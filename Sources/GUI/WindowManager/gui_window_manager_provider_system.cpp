	...
	GUITopLevelDescription description)
{
    ...
    top_level_window->window.sig_lost_focus().connect({this, &GUIWindowManagerProvider_System::on_displaywindow_lost_focus, handle});
    top_level_window->window.sig_got_focus().connect({this, &GUIWindowManagerProvider_System::on_displaywindow_got_focus, handle});
    top_level_window->window.sig_resize().connect({this, &GUIWindowManagerProvider_System::on_displaywindow_resize, handle});
    top_level_window->window.sig_paint().connect({this, &GUIWindowManagerProvider_System::on_displaywindow_paint, handle});
    top_level_window->window.sig_window_close().connect({this, &GUIWindowManagerProvider_System::on_displaywindow_window_close, handle});
    top_level_window->window.sig_window_destroy().connect({this, &GUIWindowManagerProvider_System::on_displaywindow_window_destroy, handle});

	InputContext ic = top_level_window->window.get_ic();
    ic.get_mouse().sig_key_up().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
    ic.get_mouse().sig_key_down().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
    ic.get_mouse().sig_key_dblclk().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
    ic.get_mouse().sig_pointer_move().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
    ic.get_keyboard().sig_key_up().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
    ic.get_keyboard().sig_key_down().connect({this, &GUIWindowManagerProvider_System::on_input, handle});

	for (int i = 0; i < ic.get_tablet_count(); ++i)
	{
        ic.get_tablet(i).sig_axis_move().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
        ic.get_tablet(i).sig_key_down().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
        ic.get_tablet(i).sig_key_dblclk().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
        ic.get_tablet(i).sig_key_up().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
        ic.get_tablet(i).sig_proximity_change().connect({this, &GUIWindowManagerProvider_System::on_input, handle});
	}
