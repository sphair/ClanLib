...
void Frame_Impl::on_render(Canvas &canvas, const Rect &update_rect)
{
	...
	Rect rect_header(content.left, content.top, content.right, content.top + part_header.get_css_height());
	...
	{
		content_caption_width += frame->get_render_text_box(canvas, header_text).get_width();
	}
