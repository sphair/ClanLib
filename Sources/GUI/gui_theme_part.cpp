...
Font GUIThemePart::get_font() const
{
	...
	return impl->element.get_font(canvas, impl->component->get_resources());
}
...
void GUIThemePart::set_style(const CSSPropertyValue &value, bool enable)
{
...
{
	Font font = impl->element.get_font(canvas, impl->component->get_resources());
...
{
	return impl->element.get_render_text_box(canvas, str, content_rect, impl->component->get_resources());
}
...
Size GUIThemePart::get_render_text_size( Canvas &canvas, const std::string &str ) const
{
	...
	return impl->element.get_render_text_box(canvas, str, impl->component->get_content_box(), impl->component->get_resources()).get_size();
}
