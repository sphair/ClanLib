...
inline void VertexAttributeFetcherPtr::throw_if_insufficient_buffer(size_t size)
{
	if (alloc_buffer_size < size)
		throw CL_Exception(cl_text("Insufficient buffer size for vertex attribute fetcher object"));
}
...
inline const void *VertexAttributeFetcher::find_vertex_data(int index)
{
	if (data == 0)
	{
		...
		{
			// CL_GDIVertexArrayBufferProvider *array_provider = static_cast<CL_GDIVertexArrayBufferProvider*>(vertex_data.array_provider);
