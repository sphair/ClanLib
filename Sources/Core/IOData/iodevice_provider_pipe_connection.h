...
public:
	IODeviceProvider_PipeConnection(const std::string &pipe_name);
#ifdef WIN32
	IODeviceProvider_PipeConnection(HANDLE pipe_handle);
#else
	IODeviceProvider_PipeConnection(int pipe_handle);
#endif

	~IODeviceProvider_PipeConnection();
