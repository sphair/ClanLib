...
/////////////////////////////////////////////////////////////////////////////
// IODeviceProvider_PipeConnection Construction:

#ifdef WIN32
IODeviceProvider_PipeConnection::IODeviceProvider_PipeConnection(
	const std::string &pipe_name)
: handle(INVALID_HANDLE_VALUE), peeked_data(0)
{
	handle = CreateFile(
		StringHelp::utf8_to_ucs2("\\\\.\\pipe\\" + pipe_name).c_str(),
	...
	if (handle == INVALID_HANDLE_VALUE)
		throw Exception(string_format("Unable to open pipe %1", pipe_name));
	...
	{
		CloseHandle(handle);
		throw Exception(string_format("Unable to set named pipe handle state on %1", pipe_name));
	}
}
#else
IODeviceProvider_PipeConnection::IODeviceProvider_PipeConnection(
...
{
	if (handle == INVALID_HANDLE_VALUE)
		throw Exception("Invalid handle value");
}
#else
IODeviceProvider_PipeConnection::IODeviceProvider_PipeConnection(
	int pipe_handle)
...
{
	if (handle == -1)
		throw Exception("Invalid socket value");
}
#endif
...
/////////////////////////////////////////////////////////////////////////////
// IODeviceProvider_PipeConnection Attributes:

/////////////////////////////////////////////////////////////////////////////
// IODeviceProvider_PipeConnection Operations:

int IODeviceProvider_PipeConnection::send(const void *_data, int length, bool send_all)
{
#ifdef WIN32
	...
	while (pos < length)
	{
		...
		if (result == FALSE && error != ERROR_IO_PENDING)
			throw Exception("Write failed to pipe!");
		DWORD written = 0;
	...
	while (pos < length)
	{
		...
		if (res == -1)
			throw Exception("Write failed to socket!");
		pos += res;
		if (!send_all)
			break;
	}
...
void IODeviceProvider_PipeConnection::disconnect()
{
#ifdef WIN32
	if (handle != INVALID_HANDLE_VALUE)
		CloseHandle(handle);
...
IODeviceProvider *IODeviceProvider_PipeConnection::duplicate()
{
	throw Exception("IODeviceProvider_PipeConnection::duplicate() - Not Implemented");
}

/////////////////////////////////////////////////////////////////////////////
// IODeviceProvider_PipeConnection Implementation:

int IODeviceProvider_PipeConnection::lowlevel_read(void *buffer, int length, bool receive_all)
{
#ifdef WIN32
	...
	while (received < length)
	{
		...
		DWORD error = (result == FALSE) ? GetLastError() : ERROR_SUCCESS;
		if (result == FALSE && error != ERROR_IO_PENDING)
			throw Exception("Read failed from pipe!");
		DWORD bytes_read = 0;
	...
	while (received < length)
	{
		...
		if (res == -1)
			throw Exception("Read failed on socket!");
		received += res;
		if (!receive_all)
			break;
	}
