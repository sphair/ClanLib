...
/////////////////////////////////////////////////////////////////////////////
// ZipIODevice_FileEntry construction:
...
/////////////////////////////////////////////////////////////////////////////
// ZipIODevice_FileEntry attributes:
...
/////////////////////////////////////////////////////////////////////////////
// ZipIODevice_FileEntry operations:
...
bool ZipIODevice_FileEntry::seek(int seek_pos, IODevice::SeekMode mode)
{
	...
	case zip_compress_deflate:
		...
		{
			int received = receive(buffer, int(min(absolute_pos-pos, 1024)), true);
			if (received == 0) break;
		}
		break;
...
/////////////////////////////////////////////////////////////////////////////
// ZipIODevice_FileEntry implementation:
...
int ZipIODevice_FileEntry::lowlevel_read(void *data, int size, bool read_all)
{
	...
	case zip_compress_store: // no compression
		{
			int received = iodevice.receive(data, int(min(size, file_header.uncompressed_size - pos)), read_all);
			pos += received;
			return received;
		}
		break;
	...
	case zip_compress_deflate:
		...
		while (zs.avail_out > 0)
		{
			...
			if (zs.avail_in == 0 && compressed_pos < file_header.compressed_size)
			{
				...
				{
					received_input += iodevice.receive(zbuffer, int(min(16*1024, file_header.compressed_size - compressed_pos)), true);
