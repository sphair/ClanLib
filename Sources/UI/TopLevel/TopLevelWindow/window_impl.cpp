...
namespace clan
{
	Window_Impl::Window_Impl(Window *view, const DisplayWindowDescription &desc) : window_view(view), window(desc)
	{
		canvas = Canvas(window);

		slots.connect(window.sig_lost_focus(), clan::bind_member(this, &Window_Impl::on_lost_focus));
		slots.connect(window.sig_got_focus(), clan::bind_member(this, &Window_Impl::on_got_focus));
		slots.connect(window.sig_resize(), clan::bind_member(this, &Window_Impl::on_resize));
		slots.connect(window.sig_paint(), clan::bind_member(this, &Window_Impl::on_paint));
		slots.connect(window.sig_window_close(), clan::bind_member(this, &Window_Impl::on_window_close));
		slots.connect(window.get_ic().get_keyboard().sig_key_down(), clan::bind_member(this, &Window_Impl::on_key_down));
		slots.connect(window.get_ic().get_keyboard().sig_key_up(), clan::bind_member(this, &Window_Impl::on_key_up));
		slots.connect(window.get_ic().get_mouse().sig_key_down(), clan::bind_member(this, &Window_Impl::on_mouse_down));
		slots.connect(window.get_ic().get_mouse().sig_key_dblclk(), clan::bind_member(this, &Window_Impl::on_mouse_dblclk));
		slots.connect(window.get_ic().get_mouse().sig_key_up(), clan::bind_member(this, &Window_Impl::on_mouse_up));
		slots.connect(window.get_ic().get_mouse().sig_pointer_move(), clan::bind_member(this, &Window_Impl::on_mouse_move));
	}

	void Window_Impl::on_lost_focus()
	{
		release_capture();
		popups.clear();
	...
	void Window_Impl::on_got_focus()
	{
		release_capture();
		window_view->dispatch_activation_change(ActivationChangeType::activated);
	}

	void Window_Impl::on_resize(float, float)
	{
		window.request_repaint(window.get_viewport());
	}

	void Window_Impl::on_paint(const clan::Rect &box)
	{
	...
	void Window_Impl::dispatch_hot_event(std::shared_ptr<View> &view, PointerEvent &e)
	{
		if (view != hot_view)
		{
			if (hot_view)
	...
	void Window_Impl::release_capture()
	{
		if (captured_view)
		{
	...
	std::shared_ptr<View> Window_Impl::get_capture_view(PointerEvent &e, std::shared_ptr<View> &view_above_cursor)
	{
	...
	void Window_Impl::on_key_up(const clan::InputEvent &e)
	{
	...
	void Window_Impl::on_mouse_down(const clan::InputEvent &e)
	{
	...
	void Window_Impl::on_mouse_dblclk(const clan::InputEvent &e)
	{
	...
	void Window_Impl::on_mouse_up(const clan::InputEvent &e)
	{
	...
	void Window_Impl::on_mouse_move(const clan::InputEvent &clan_event)
	{
	...
	PointerButton Window_Impl::decode_id(clan::InputCode ic) const
	{
		switch (ic)
		{
		default:
