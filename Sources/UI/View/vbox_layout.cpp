...
namespace clan
{
	...
	void VBoxLayout::layout_subviews(Canvas &canvas, View *view)
	{
		...
		for (const std::shared_ptr<View> &subview : view->subviews())
		{
			if (subview->style()->computed_value("position").is_keyword("static") && !subview->hidden())
			{
				noncontent_height += subview->style()->computed_value("margin-top").number;
				noncontent_height += subview->style()->computed_value("border-top-width").number;
				noncontent_height += subview->style()->computed_value("padding-top").number;
				noncontent_height += subview->style()->computed_value("padding-bottom").number;
				noncontent_height += subview->style()->computed_value("border-bottom-width").number;
				noncontent_height += subview->style()->computed_value("margin-bottom").number;
		...
		for (const std::shared_ptr<View> &subview : view->subviews())
		{
			if (subview->style()->computed_value("position").is_keyword("static") && !subview->hidden())
			{
				...
				if (free_space < 0.0f && total_shrink_factor != 0.0f)
					subview_height += subview->style()->computed_value("flex-shrink").number * free_space / total_shrink_factor;
				else if (free_space > 0.0f && total_grow_factor != 0.0f)
					subview_height += subview->style()->computed_value("flex-grow").number * free_space / total_grow_factor;
