...
namespace clan
{
	...
	{
		auto style_width = view->style_cascade().computed_value("width");
	...
	{
		auto style_height = view->style_cascade().computed_value("height");
	...
	void BlockLayout::layout_subviews(Canvas &canvas, View *view)
	{
		...
		for (const std::shared_ptr<View> &subview : view->subviews())
		{
			if (subview->style_cascade().computed_value("position").is_keyword("static") && !subview->hidden())
			{
				...
				subview->set_geometry(BoxGeometry::from_content_box(subview->style_cascade(), Rectf::xywh(left_noncontent, y, subview_width, subview_height)));
