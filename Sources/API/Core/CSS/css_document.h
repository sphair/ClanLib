...
public:
	...
	/** <p>The syntax of element is the same as specifying contextual elements
	    ...
	    first in the list.</p>*/
	std::vector<CSSProperty> select(const std::string &element);
	...
	static std::vector<CSSProperty> parse_style_properties(const std::string &style_text);
/// \}
