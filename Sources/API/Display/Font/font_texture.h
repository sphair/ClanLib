...
typedef struct _CL_Font_Texture_Position
{
	...
	int y_increment;
} CL_Font_Texture_Position;

/// \brief Bitmap Font Glyph class.
///
/// \xmlonly !group=Display/Font! !header=display.h! \endxmlonly
class CL_API_DISPLAY CL_Font_Texture_Glyph
{
public:
	CL_Font_Texture_Glyph() : glyph(0), empty_buffer(true), offset(0,0), increment(0,0) { };
...
///
/// \xmlonly !group=Display/Font! !header=display.h! \endxmlonly
class CL_API_DISPLAY CL_Font_Texture : public CL_Font
{
/// \name Construction
/// \{
...
public:
	...
	/** param: context = The graphic context
	    param: pixel_buffer = The font pixel buffer. NULL = Not required
	    param: positions = The bitmap font positions. NULL = Not required
	    param: num_positions = The number of glyphs in the positions (-1 = Use the positions end of list marker instead)*/
	CL_Font_Texture( CL_GraphicContext context, CL_PixelBuffer *pixel_buffer = NULL, CL_Font_Texture_Position *positions = NULL, int num_positions = -1 );

	CL_Font_Texture( CL_GraphicContext context, const CL_StringRef &typeface_name, int height);

	CL_Font_Texture( CL_GraphicContext context, const CL_FontDescription &desc);
...
public:
	...
	CL_FontProvider_Texture *get_provider();

	/// \brief Get a glyph. Returns NULL if the glyph was not found
	CL_Font_Texture_Glyph *get_glyph(const CL_GraphicContext &gc, int glyph);
...
public:
	...
	void insert_glyph(const CL_GraphicContext &gc, CL_Font_Texture_Position &position, CL_PixelBuffer &pixel_buffer);

	/// \brief Insert glyphs from a text string (using to system font)
	/** param: gc = The graphic context
	    param: text = The text to use*/
	void insert_glyph(const CL_GraphicContext &gc, const CL_StringRef &text);
