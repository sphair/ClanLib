...
public:
	OutlineProviderBitmap_Impl(const PixelBuffer &pbuf, int alpha_limit=128, bool get_insides=true);
...
private:
	...
	Pointf find_next_contour_start(unsigned int x, unsigned int y);

	// add the next point
	void get_next_point(std::vector<Pointf> &points);
