	...
	CL_ResourceManager *resources)
{
	...
	while (!node.is_null())
	{
		if (node.is_element())
		{
			...
			if (element.get_tag_name() == cl_text("shader"))
			{
				CL_ShaderObject shader = CL_ShaderObject::load(gc, element.get_attribute(cl_text("name")), resources);
				program_object.attach(shader);
			}
			else if (element.get_tag_name() == cl_text("bind-attribute"))
			{
				program_object.bind_attribute_location(
					CL_StringHelp::text_to_int(element.get_attribute(cl_text("index"))),
					element.get_attribute(cl_text("name")));
			}
		}
		node = node.get_next_sibling();
	}
	
	if (!resource.get_element().get_attribute(cl_text("shader")).empty())
	{
		CL_ShaderObject shader = CL_ShaderObject::load(gc, resource.get_element().get_attribute(cl_text("shader")), resources);
		program_object.attach(shader);
	}
	
	if (resource.get_element().get_attribute(cl_text("link"), cl_text("true")) == cl_text("true"))
		if(!program_object.link())
			throw CL_Exception(cl_format(cl_text("Unable to link program object: %1"), program_object.get_info_log()));

	return program_object;
}

CL_ProgramObject CL_ProgramObject::load(CL_GraphicContext &gc, const CL_StringRef &vertex_filename, const CL_StringRef &fragment_filename, CL_VirtualDirectory &directory)
{
	CL_ProgramObject program_object(gc);
...
CL_ProgramObject CL_ProgramObject::load(CL_GraphicContext &gc, const CL_StringRef &fragment_filename, CL_VirtualDirectory &directory)
{
	CL_ProgramObject program_object(gc);
...
CL_ProgramObject CL_ProgramObject::load_and_link(CL_GraphicContext &gc, const CL_StringRef &vertex_filename, const CL_StringRef &fragment_filename, CL_VirtualDirectory &directory)
{
...
CL_ProgramObject CL_ProgramObject::load_and_link(CL_GraphicContext &gc, const CL_StringRef &fragment_filename, CL_VirtualDirectory &directory)
{
