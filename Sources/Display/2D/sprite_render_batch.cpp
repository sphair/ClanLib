...
void CL_SpriteRenderBatch::draw_sprite(CL_GraphicContext &gc, const CL_Surface_DrawParams1 *params, const CL_Texture &texture)
{
...
void CL_SpriteRenderBatch::draw_image(CL_GraphicContext &gc, const CL_Rectf &src, const CL_Rectf &dest, const CL_Colorf &color, const CL_Texture &texture)
{
	int texindex = set_batcher_active(gc, texture);
...
void CL_SpriteRenderBatch::fill(CL_GraphicContext &gc, float x1, float y1, float x2, float y2, const CL_Colorf &color)
{
...
inline CL_Vec3f CL_SpriteRenderBatch::to_position(float x, float y) const
{
	return CL_Vec3f(
...
int CL_SpriteRenderBatch::set_batcher_active(CL_GraphicContext &gc, const CL_Texture &texture)
{
	int texindex = -1;
	...
	if (texindex == -1 && num_current_textures < max_textures)
	{
		...
		tex_sizes[texindex] = CL_Sizef((float)current_textures[texindex].get_width(0), (float)current_textures[texindex].get_height(0));
	}
...
int CL_SpriteRenderBatch::set_batcher_active(CL_GraphicContext &gc)
{
...
void CL_SpriteRenderBatch::flush(CL_GraphicContext &gc)
{
	if (position > 0)
	{
...
void CL_SpriteRenderBatch::modelview_changed(const CL_Mat4f &new_modelview)
{
