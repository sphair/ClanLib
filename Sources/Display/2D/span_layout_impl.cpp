...
void SpanLayout_Impl::draw_layout_text(Canvas &canvas, Line &line, LineSegment &segment, int x, int y)
{
	...
	if (s1 != s2)
	{
		...
		{
			if (is_ellipsis_draw)
				segment.font.draw_text_ellipsis(canvas, xx, y + line.ascender, ellipsis_content_rect, segment_text.substr(0, s1), segment.color);
			else
				segment.font.draw_text(canvas, xx, y + line.ascender, segment_text.substr(0, s1), segment.color);
		}
		if (is_ellipsis_draw)
			segment.font.draw_text_ellipsis(canvas, xx0, y+line.ascender, ellipsis_content_rect, segment_text.substr(s1, s2 - s1), sel_foreground);
		else
		...
		{
			if (is_ellipsis_draw)
				segment.font.draw_text_ellipsis(canvas, xx1, y + line.ascender, ellipsis_content_rect, segment_text.substr(s2), segment.color);
			else
	...
	else
	{
		...
		if (is_ellipsis_draw)
			segment.font.draw_text_ellipsis(canvas, x + segment.x_position, y + line.ascender, ellipsis_content_rect, segment_text, segment.color);
		else
