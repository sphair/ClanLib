...
const std::vector<ImageDescriptionFrame> &ImageDescription::get_frames() const
{
	return impl->frames;
}
...
void ImageDescription::add_frame(const PixelBuffer &pixelbuffer)
{
	impl->frames.push_back(ImageDescriptionFrame(pixelbuffer, Rect(0, 0, pixelbuffer.get_width(), pixelbuffer.get_height())));
}

void ImageDescription::add_frame(const Texture2D &texture)
{
	impl->frames.push_back(ImageDescriptionFrame(texture, Rect(0, 0, texture.get_width(), texture.get_height())));
}

void ImageDescription::add_frame(const std::string &filename, VirtualDirectory &dir, const ImageImportDescription &import_desc)
{
...
void ImageDescription::add_frame(const std::string &fullname, const ImageImportDescription &import_desc)
{
...
void ImageDescription::add_frame(IODevice &file, const std::string &image_type, const ImageImportDescription &import_desc)
{
...
void ImageDescription::add_frames(const Texture2D &texture, Rect *frames, int num_frames)
{
	for(int i=0; i<num_frames; ++i)
		impl->frames.push_back(ImageDescriptionFrame(texture, frames[i]));
}

void ImageDescription::add_gridclipped_frames(
	...
	int xspace, int yspace)
{
	...
	for(int y = 0; y < yarray; y++)
	{
		...
		for(int x = 0; x < xarray; x++)
		{
			...
			impl->frames.push_back(ImageDescriptionFrame(pixelbuffer, Rect(xstart, ystart, xstart + width, ystart + height)));
			xstart += width + xspace;
		}
...
void ImageDescription::add_gridclipped_frames(GraphicContext &gc, 
	...
	int xspace, int yspace)
{
	...
	for(int y = 0; y < yarray; y++)
	{
		...
		for(int x = 0; x < xarray; x++)
		{
			...
			impl->frames.push_back(ImageDescriptionFrame(texture, Rect(xstart, ystart, xstart + width, ystart + height)));
			xstart += width + xspace;
		}
		ystart += height + yspace;
	}
...
void ImageDescription::add_alphaclipped_frames(GraphicContext &gc, 
	const Texture2D &texture, 
	int xpos, int ypos, 
	float trans_limit)
{
...
void ImageDescription::add_alphaclipped_frames(
	const PixelBuffer &pixelbuffer, 
	int xpos, int ypos, 
	float trans_limit)
{
	...
	for(int x=xpos; x < alpha_width; x++)
	{
		...
		else if (!opaque_row[x] && !prev_trans)
		{
			impl->frames.push_back(
				ImageDescriptionFrame(pixelbuffer, Rect(begin, cut_top, x+1, cut_bottom)));
	...
	{
		impl->frames.push_back(
			ImageDescriptionFrame(pixelbuffer, Rect(begin, cut_top, alpha_width, cut_bottom)));
	}
}

void ImageDescription::add_alphaclipped_frames_free(GraphicContext &gc, 
	const Texture2D &texture, 
	int xpos, int ypos, 
	float trans_limit)
{
...
void ImageDescription::add_alphaclipped_frames_free(
	const PixelBuffer &pixelbuffer, 
	int xpos, int ypos, 
	float trans_limit)
{
	...
	for (int y=ypos; y < height; y++)
	{
		for (int x=xpos; x < width; x++)
		{
			...
			impl->frames.push_back(ImageDescriptionFrame(pixelbuffer, Rect(x1, y1, x2, y2)));
		}
	}
}

void ImageDescription::set_frame_delay(int frame, double delay)
{
