...
class CL_PerlinNoise_PixelWriter
{
public:
...
class CL_PerlinNoise_PixelWriter_RGBA8 : public CL_PerlinNoise_PixelWriter
{
public:
	CL_PerlinNoise_PixelWriter_RGBA8(CL_PixelBuffer &pbuff) 
		: pitch(pbuff.get_pitch() / pbuff.get_bytes_per_pixel()),
		  current_ptr((cl_ubyte32 *) pbuff.get_data()),
...
private:
	int pitch;
	cl_ubyte32 *current_ptr;
	cl_ubyte32 *line_start_ptr;
};

class CL_PerlinNoise_PixelWriter_RGB8 : public CL_PerlinNoise_PixelWriter
{
public:
	CL_PerlinNoise_PixelWriter_RGB8(CL_PixelBuffer &pbuff) 
		: pitch(pbuff.get_pitch()),
		  current_ptr((cl_ubyte8 *) pbuff.get_data()),
		  line_start_ptr(current_ptr)
	{
	}
...
private:
	int pitch;
	cl_ubyte8 *current_ptr;
	cl_ubyte8 *line_start_ptr;
};

class CL_PerlinNoise_PixelWriter_R8 : public CL_PerlinNoise_PixelWriter
{
public:
	CL_PerlinNoise_PixelWriter_R8(CL_PixelBuffer &pbuff) 
		: pitch(pbuff.get_pitch()),
		  current_ptr((cl_ubyte8 *) pbuff.get_data()),
		  line_start_ptr(current_ptr)
	{
...
class CL_PerlinNoise_PixelWriter_R32f : public CL_PerlinNoise_PixelWriter
{
public:
	CL_PerlinNoise_PixelWriter_R32f(CL_PixelBuffer &pbuff) 
...
private:
	void create_noise4d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x, float start_y, float end_y, float z_position, float w_position);
	void create_noise3d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x, float start_y, float end_y, float z_position);
	void create_noise2d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x, float start_y, float end_y);
	void create_noise1d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x);
...
CL_PerlinNoise::CL_PerlinNoise() : impl(new CL_PerlinNoise_Impl)
{
}
...
void CL_PerlinNoise::set_permutations(const unsigned char *table, unsigned int size)
{
	impl->set_permutations(table, size);
}

CL_PixelBuffer CL_PerlinNoise::create_noise1d(float start_x, float end_x)
{
	return impl->create_noise1d(start_x, end_x);
}

CL_PixelBuffer CL_PerlinNoise::create_noise2d(float start_x, float end_x, float start_y, float end_y)
{
	return impl->create_noise2d(start_x, end_x, start_y, end_y);
}

CL_PixelBuffer CL_PerlinNoise::create_noise3d(float start_x, float end_x, float start_y, float end_y, float z_position)
{
	return impl->create_noise3d(start_x, end_x, start_y, end_y, z_position);
}

CL_PixelBuffer CL_PerlinNoise::create_noise4d(float start_x, float end_x, float start_y, float end_y, float z_position, float w_position)
{
...
void CL_PerlinNoise::set_size(int width, int height)
{
	impl->width = width;
	impl->height = height;
}
...
void CL_PerlinNoise::set_format(CL_TextureFormat sized_format)
{
	impl->sized_format = sized_format;
}

void CL_PerlinNoise::set_amplitude(float amplitude)
{
	impl->amplitude = amplitude;
}
void CL_PerlinNoise::set_octaves(int octaves)
{
	impl->octaves = octaves;
}

float CL_PerlinNoise_Impl::gradient_1d( int permutation_value, float x )
{
...
float CL_PerlinNoise_Impl::gradient_2d( int permutation_value, float x, float y )
{
	float u,v;
...
float CL_PerlinNoise_Impl::gradient_3d( int permutation_value, float x, float y , float z)
{
...
float CL_PerlinNoise_Impl::gradient_4d( int permutation_value, float x, float y, float z, float t )
{
	float u,v,w;
...
float CL_PerlinNoise_Impl::noise_1d( float x )
{
	int ix0, ix1;
	float fx0, fx1;
	float s, n0, n1;
...
float CL_PerlinNoise_Impl::noise_2d( float x, float y )
{
	int ix0, iy0, ix1, iy1;
...
float CL_PerlinNoise_Impl::noise_3d( float x, float y, float z )
{
...
float CL_PerlinNoise_Impl::noise_4d( float x, float y, float z, float w )
{
...
void CL_PerlinNoise_Impl::set_permutations(const unsigned char *table, unsigned int size)
{
...
void CL_PerlinNoise_Impl::setup()
{
...
CL_PixelBuffer CL_PerlinNoise_Impl::create_noise2d(float start_x, float end_x, float start_y, float end_y)
{
	setup();

	if (sized_format == cl_rgba8)
	{
		CL_PixelBuffer pbuff(width, height, sized_format);
		CL_PerlinNoise_PixelWriter_RGBA8 writer(pbuff);
		create_noise2d(writer, start_x, end_x, start_y, end_y);
		return pbuff;
	}
	if (sized_format == cl_rgb8)
	{
		CL_PixelBuffer pbuff(width, height, sized_format);
		CL_PerlinNoise_PixelWriter_RGB8 writer(pbuff);
		create_noise2d(writer, start_x, end_x, start_y, end_y);
		return pbuff;
	}
	if (sized_format == cl_r8)
	{
		CL_PixelBuffer pbuff(width, height, sized_format);
		CL_PerlinNoise_PixelWriter_R8 writer(pbuff);
		create_noise2d(writer, start_x, end_x, start_y, end_y);
		return pbuff;
	}
	if (sized_format == cl_r32f)
	{
		CL_PixelBuffer pbuff(width, height, sized_format);
		CL_PerlinNoise_PixelWriter_R32f writer(pbuff);
		create_noise2d(writer, start_x, end_x, start_y, end_y);
		return pbuff;
	}

	throw CL_Exception("sized format is not supported");

}

void CL_PerlinNoise_Impl::create_noise2d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x, float start_y, float end_y)
{
...
CL_PixelBuffer CL_PerlinNoise_Impl::create_noise1d(float start_x, float end_x)
{
	setup();
...
void CL_PerlinNoise_Impl::create_noise1d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x)
{
	float size_x = end_x - start_x;
	float fheight = (float) height;
	float fwidth = (float) width;
...
CL_PixelBuffer CL_PerlinNoise_Impl::create_noise3d(float start_x, float end_x, float start_y, float end_y, float z_position)
{
	setup();
...
void CL_PerlinNoise_Impl::create_noise3d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x, float start_y, float end_y, float z_position)
{
	float size_x = end_x - start_x;
	float size_y = end_y - start_y;
	float fheight = (float) height;
...
CL_PixelBuffer CL_PerlinNoise_Impl::create_noise4d(float start_x, float end_x, float start_y, float end_y, float z_position, float w_position)
{
	setup();
...
void CL_PerlinNoise_Impl::create_noise4d(CL_PerlinNoise_PixelWriter &writer, float start_x, float end_x, float start_y, float end_y, float z_position, float w_position)
{
	float size_x = end_x - start_x;
	float size_y = end_y - start_y;
	float fheight = (float) height;
