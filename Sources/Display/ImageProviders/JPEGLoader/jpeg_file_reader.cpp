...
{
	cl_uchar b1 = iodevice.read_uint8();
	if (b1 != 0xff)
...
{
	cl_ushort size = iodevice.read_uint16();
...
CL_JPEGStartOfFrame CL_JPEGFileReader::read_sof()
{
	...
	cl_uchar num_components = iodevice.read_uint8();
	...
	for (int i = 0; i < num_components; i++)
	{
		...
		cl_uchar sampling_factor = iodevice.read_uint8();
...
CL_JPEGDefineQuantizationTable CL_JPEGFileReader::read_dqt()
{
	...
	while (p < size)
	{
		...
		cl_uchar v = iodevice.read_uint8();
		cl_uchar precision = (v & 0xf0) >> 4;
...
CL_JPEGStartOfScan CL_JPEGFileReader::read_sos()
{
	...
	for (int i = 0; i < num_components; i++)
	{
		...
		cl_uchar table_selector = iodevice.read_uint8();
	...
	cl_uchar ahal = iodevice.read_uint8();
...
int CL_JPEGFileReader::read_entropy_data(void *d, int size)
{
	...
	cl_uchar *data = reinterpret_cast<cl_uchar*>(d);
