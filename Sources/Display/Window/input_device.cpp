...
{
	if (impl->provider)
		return Point(impl->provider->get_x(), impl->provider->get_y());
	else
		return Point(0,0);
}

int InputDevice::get_x() const
{
	if (impl->provider)
