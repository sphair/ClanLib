...
CL_Size CL_FontProvider_Texture::get_text_size(CL_GraphicContext &gc, const CL_StringRef &text)
{
	...
	{
		CL_Font_Texture_Glyph *gptr = get_glyph(gc, text[p]);
...
CL_Font_Texture_Glyph *CL_FontProvider_Texture::get_glyph(const CL_GraphicContext &gc, unsigned int glyph)
{
	std::vector< CL_Font_Texture_Glyph * >::size_type size = glyph_list.size();
...
CL_FontPixelBuffer CL_FontProvider_Texture::get_font_glyph(const CL_GraphicContext &gc, int glyph, bool anti_alias, const CL_Colorf &color)
{
	if (!handle)
	{
...
void CL_FontProvider_Texture::insert_glyph(const CL_GraphicContext &gc, CL_Font_Texture_Position &position, CL_PixelBuffer &pixel_buffer)
{
	...
	CL_Font_Texture_Glyph *font_glyph = new(CL_Font_Texture_Glyph);
...
void CL_FontProvider_Texture::insert_glyph(const CL_GraphicContext &gc, CL_FontPixelBuffer &pb)
{
	// Search for duplicated glyph's, if found silently ignore them
	std::vector< CL_Font_Texture_Glyph * >::size_type size = glyph_list.size();
	for (int cnt=0; cnt<size; cnt++)
...
void CL_FontProvider_Texture::insert_glyph(const CL_GraphicContext &gc, int glyph)
{
...
void CL_FontProvider_Texture::insert_glyph(const CL_GraphicContext &gc, const CL_StringRef &text)
{
	for (CL_String::size_type p = 0; p < text.length(); ++p)
	{
