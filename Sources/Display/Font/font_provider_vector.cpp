...
void FontProvider_Vector::load_font(const FontDescription &desc, const VirtualDirectory &directory)
{
	IODevice file = directory.open_file_read(desc.get_typeface_name());
	load_font(desc, file);
}

void FontProvider_Vector::load_font(const FontDescription &desc, IODevice &io_dev)
{
...
{
	std::map<int, GlyphOutline*>::iterator it = char_cache.find(glyph);
	...
	float *out_interspacing_y)
{
	for( unsigned int i=0; i<text.length(); i++ )
	{
		...
		{
			GlyphOutline *outline = font_engine->load_glyph_outline(text[i]);
			outline->triangulate();
	...
	const Colorf &color)
{
	...
	for( int i=0; i<length; i++ )
	{
		...
		if( filled )
		{
			GlyphPrimitivesArray &prim_array = char_cache[glyphs[i]]->get_triarray();
		...
		{
			GlyphPrimitivesArrayOutline &prim_array_outline = char_cache[glyphs[i]]->get_outline();
