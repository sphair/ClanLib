...
public:
	...
	CL_Font_Texture_Glyph *get_glyph(const CL_GraphicContext &gc, unsigned int glyph);
...
private:
	void insert_glyph(const CL_GraphicContext &gc, int glyph);
	void insert_glyph(const CL_GraphicContext &gc, CL_FontPixelBuffer &pb);
	CL_FontPixelBuffer get_font_glyph(const CL_GraphicContext &gc, int glyph, bool anti_alias, const CL_Colorf &color);

	/// \brief Set the font metrics from the OS font
	void write_font_metrics(CL_GraphicContext &gc);

	std::vector<CL_Font_Texture_Glyph* > glyph_list;
