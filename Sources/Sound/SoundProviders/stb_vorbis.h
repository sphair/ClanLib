...
#ifndef INCLUDED_FROM_SETUPVORBIS
...
#ifndef M_CL_PI
  #define M_CL_PI  3.14159265358979323846264f  // from CRC
#endif
...
static void compute_twiddle_factors(int n, float *A, float *B, float *C)
{
   ...
   for (k=k2=0; k < n4; ++k,k2+=2) {
      A[k2  ] = (float)  cos(4*k*M_CL_PI/n);
      A[k2+1] = (float) -sin(4*k*M_CL_PI/n);
      B[k2  ] = (float)  cos((k2+1)*M_CL_PI/n/2) * 0.5f;
      B[k2+1] = (float)  sin((k2+1)*M_CL_PI/n/2) * 0.5f;
   }
   for (k=k2=0; k < n8; ++k,k2+=2) {
      C[k2  ] = (float)  cos(2*(k2+1)*M_CL_PI/n);
      C[k2+1] = (float) -sin(2*(k2+1)*M_CL_PI/n);
   }
}
...
static void compute_window(int n, float *window)
{
   int n2 = n >> 1, i;
   for (i=0; i < n2; ++i)
      window[i] = (float) sin(0.5 * M_CL_PI * square((float) sin((i - 0 + 0.5) / n2 * 0.5 * M_CL_PI)));
}
...
void inverse_mdct_slow(float *buffer, int n)
{
   ...
   for (i=0; i < n; ++i) {
      ...
      for (j=0; j < n2; ++j)
         // formula from paper:
         //acc += n/4.0f * x[j] * (float) cos(M_CL_PI / 2 / n * (2 * i + 1 + n/2.0)*(2*j+1));
         // formula from wikipedia
         //acc += 2.0f / n2 * x[j] * (float) cos(M_CL_PI/n2 * (i + 0.5 + n2/2)*(j + 0.5));
         // these are equivalent, except the formula from the paper inverts the multiplier!
         // however, what actually works is NO MULTIPLIER!?!
         //acc += 64 * 2.0f / n2 * x[j] * (float) cos(M_CL_PI/n2 * (i + 0.5 + n2/2)*(j + 0.5));
         acc += x[j] * (float) cos(M_CL_PI / 2 / n * (2 * i + 1 + n/2.0)*(2*j+1));
      buffer[i] = acc;
   }
   free(x);
}
...
void inverse_mdct_slow(float *buffer, int n, vorb *f, int blocktype)
{
   ...
   memcpy(x, buffer, sizeof(*x) * n2);
   for (i=0; i < 4*n; ++i)
      mcos[i] = (float) cos(M_CL_PI / 2 * i / n);
...
void dct_iv_slow(float *buffer, int n)
{
   ...
   for (i=0; i < 8*n; ++i)
      mcos[i] = (float) cos(M_CL_PI / 4 * i / n);
   for (i=0; i < n; ++i) {
      float acc = 0;
      for (j=0; j < n; ++j)
         acc += x[j] * mcos[((2 * i + 1)*(2*j+1)) & nmask];
         //acc += x[j] * cos(M_CL_PI / n * (i + 0.5) * (j + 0.5));
      buffer[i] = acc;
   }
   free(x);
}
...
void inverse_mdct_naive(float *buffer, int n)
{
   ...
   for (k=k2=0; k < n4; ++k,k2+=2) {
      A[k2  ] = (float)  cos(4*k*M_CL_PI/n);
      A[k2+1] = (float) -sin(4*k*M_CL_PI/n);
      B[k2  ] = (float)  cos((k2+1)*M_CL_PI/n/2);
      B[k2+1] = (float)  sin((k2+1)*M_CL_PI/n/2);
   }
   for (k=k2=0; k < n8; ++k,k2+=2) {
      C[k2  ] = (float)  cos(2*(k2+1)*M_CL_PI/n);
      C[k2+1] = (float) -sin(2*(k2+1)*M_CL_PI/n);
...
static int vorbis_seek_base(stb_vorbis *f, unsigned int sample_number, int fine)
{
   ...
   if (sample_number < f->p_first.last_decoded_sample) {
      ...
      while (p[0].page_end < p[1].page_start) {
         ...
         if (!vorbis_find_page(f, nullptr, nullptr))   return error(f, VORBIS_seek_failed);
