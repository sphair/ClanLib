...
class CL_GL1CreationHelper
{
public:
	CL_GL1CreationHelper(HWND window, HDC hdc);
	~CL_GL1CreationHelper();

	void set_multisampling_pixel_format(const CL_GL1WindowDescription &gldesc);
...
private:
	...
	typedef HGLRC (CL_GL1FUNC *ptr_wglCreateContextAttribsARB)(HDC, HGLRC, const int *);

	typedef BOOL (CL_GL1FUNC *ptr_wglGetPixelFormatAttribivEXT)(HDC, int, int, UINT, int *, int *);
	typedef BOOL (CL_GL1FUNC *ptr_wglGetPixelFormatAttribfvEXT)(HDC, int, int, UINT, int *, FLOAT *);
	typedef BOOL (CL_GL1FUNC *ptr_wglChoosePixelFormatEXT)(HDC, const int *, const FLOAT *, UINT, int *, UINT *);
};
