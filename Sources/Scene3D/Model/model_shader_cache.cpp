...
void ModelShaderCache::create_gbuffer_commands(GraphicContext &gc, Model *model, int level)
{
	...
	if (!model_data->mesh_lods[level].meshes.empty() && !model_data->mesh_lods[level].meshes[0].material_ranges.empty())
	{
		is_two_sided = model_data->mesh_lods[level].meshes[0].material_ranges[0].two_sided;
	...
	for (size_t i = 0; i < mesh_buffers.size(); i++)
	{
		...
		for (size_t j = 0; j < model_data->mesh_lods[level].meshes[i].material_ranges.size(); j++)
		{
