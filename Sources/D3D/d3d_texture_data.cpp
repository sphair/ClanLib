...
D3DTextureData::DeviceHandles &D3DTextureData::get_handles(const ComPtr<ID3D11Device> &device) const
{
	...
	if (texture_type == D3DTextureData::cl_ID3D11Texture1D)
	{
	...
	else if (texture_type == D3DTextureData::cl_ID3D11Texture2D)
	{
...
D3DTextureData::DeviceTextureType D3DTextureData::decode_texture_type(TextureDimensions texture_dimensions) const
{
	...
	switch (texture_dimensions)
	{
		...
		case texture_1d_array:
			texture_type = cl_ID3D11Texture1D;
			break;
		case texture_2d:
		case texture_2d_array:
		case texture_cube:
		case texture_cube_array:
			texture_type = cl_ID3D11Texture2D;
			break;
		case texture_3d:
			texture_type = cl_ID3D11Texture3D;
			break;
		default:
