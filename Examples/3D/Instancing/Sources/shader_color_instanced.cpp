...
void ShaderColorInstanced::Use(CL_GraphicContext &gc)
{
	if (!material_updated)
	{
...
void ShaderColorInstanced::SetCenters(std::vector<CL_Vec3f> &centers)
{
	program_object.set_uniformfv("Centers", 3, centers.size(), &centers[0].x);
}

void ShaderColorInstanced::SetMaterialAmbientSpecular(std::vector<CL_Vec4f> &colors)
{
	program_object.set_uniformfv("MaterialAmbientSpecular", 4, colors.size(), &colors[0].x);
}

void ShaderColorInstanced::SetMaterial(float new_material_shininess, const CL_Vec4f &new_material_emission)
{
