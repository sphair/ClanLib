...
void Model::Draw(GraphicContext &gc, GraphicStore *gs, const Mat4f &modelview_matrix, int instance_count)
{
...
void Model_Impl::Draw(GraphicContext &gc, GraphicStore *gs, const Mat4f &modelview_matrix, int instance_count)
{
	...
	else
	{
		if (!vbo_texcoords.is_null())
			throw Exception("Shader is not available");

		gs->shader_color_instanced.SetMaterial(material_shininess, material_emission);
		gs->shader_color_instanced.Use(gc, modelview_matrix, matrix_modelview_projection, Mat4f(normal_matrix));
