...
char ShaderTexture::vertex[] =
	...
	"	ShadowCoord = ShadowMatrix * in_position;\n"
	"}\n"
	;
...
char ShaderTexture::fragment[] =
	...
	"\n"
	"	vec4 sc = ((ShadowCoord/ShadowCoord.w) + 1.0) * 0.5;\n"
	"	vec4 shadow = shadow2D(Shadow1, sc.xyz);\n"
	"\n"
	"	if(sc.x >= 0.0 && sc.x <= 1.0 && sc.y >= 0.0 && sc.y <= 1.0 && (shadow.z+0.0005) < sc.z)\n"
	"	{\n"
	"			gl_FragColor = texture_color * ambient_light;\n"
	"	}\n"
	"	else\n"
	"	{\n"
...
ShaderTexture::ShaderTexture(CL_GraphicContext &gc)
{
	...
	program_object.set_uniform1i("Shadow1", 1);
...
void ShaderTexture::SetShadowMatrix(const CL_Mat4f &shadow_matrix)
{
	program_object.set_uniform_matrix("ShadowMatrix", shadow_matrix);
}
