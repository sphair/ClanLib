...
void TextExplosion2D::explode(CL_GraphicContext &gc, const CL_String &new_text, int new_x, int new_y, CL_Font new_font, CL_Colorf new_color)
{
...
void TextExplosion2D::render(CL_GraphicContext &gc)
{
	if (!letter_positions.empty())
	{
		...
		{
			font.draw_text(gc, (int)letter_positions[i].x, (int)letter_positions[i].y, text.substr(i, 1), color);

			letter_positions[i] += letter_deltas[i] * time_elapsed * 5.0f;
			letter_deltas[i].y += time_elapsed * 20.0f;
			if (letter_deltas[i].y < gc.get_height())
				still_visible = true;
		}
		if (!still_visible)
		{
