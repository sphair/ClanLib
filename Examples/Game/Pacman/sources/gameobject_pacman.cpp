	...
	GameObject_Moving(x, y, world)
{
	...
	//sfx_powerup = SoundBuffer("Game/sfx_powerup", world->resources);
	//sfx_dead = SoundBuffer("Game/sfx_dead", world->resources);
...
{
	if (got_powerup && System::get_time() - powerup_starttime>20000)
	{
		got_powerup = false;
	}
...
{
	cc.connect(window.get_ic().get_keyboard().sig_key_down(), std::function<void(const clan::InputEvent&)>(this, &GameObject_Pacman::on_key_down));
}
...
//		sfx_namnam.play();
	...
	if (world->map->get_tile_type(dest_x, dest_y) == Map::tile_powerup) // powerup
	{
		...
		world->map->eat_egg(dest_x, dest_y);
		got_powerup = true;
		powerup_starttime = System::get_time();
//		sfx_powerup.play();
	}
...
/*
	if (move_dir < 2)
	{
		if (Keyboard::get_keycode(KEY_UP) && verify_move(0, -1))
			move_dir = 2;

		if (Keyboard::get_keycode(KEY_DOWN) && verify_move(0, 1))
			move_dir = 3;
	}
	else
	{
		if (Keyboard::get_keycode(KEY_LEFT) && verify_move(-1, 0))
			move_dir = 0;

		if (Keyboard::get_keycode(KEY_RIGHT) && verify_move(1, 0))
			move_dir = 1;
	}
*/
...
void GameObject_Pacman::on_key_down(const InputEvent &key)
{
	switch (move_dir)
	{
	case 0:
