...
{
	spr_ghost = Sprite(world->gc, "Game/spr_ghost", world->resources);
...
bool GameObject_Ghost::event_reached_dest()
{
	...
	if (world->map->get_tile_type(dest_x, dest_y) == Map::tile_trail)
	{
		if (world->player != NULL)
		{
			...
			{
				float vol = 1/div;
				//SoundBuffer_Session ses = sfx_ghost.prepare();
				//ses.set_volume(vol);
				//ses.play();
			}
		}
...
//	if (follow_trail) Console::write_line("ghost in follow trail mode");
