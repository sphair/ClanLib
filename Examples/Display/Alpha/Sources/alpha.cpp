...
int Alpha::start(const std::vector<std::string> &args)
{
	...
	cc.connect(window.sig_window_close(), std::function<void()>(this, &Alpha::on_window_close));

	// Connect a keyboard handler to on_key_up()
	cc.connect(window.get_ic().get_keyboard().sig_key_up(), std::function<void(const clan::InputEvent&)>(this, &Alpha::on_input_up));
