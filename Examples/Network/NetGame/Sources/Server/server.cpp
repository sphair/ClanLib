...
: next_user_id(1)
{
	...
	cc.connect(network_server.sig_client_connected(), std::function<void(NetGameConnection*)>(this, &Server::on_client_connected));
	cc.connect(network_server.sig_client_disconnected(), std::function<void(NetGameConnection*, const std::string&)>(this, &Server::on_client_disconnected));
	cc.connect(network_server.sig_event_received(), std::function<void(NetGameConnection*, const NetGameEvent&)>(this, &Server::on_event_received));
