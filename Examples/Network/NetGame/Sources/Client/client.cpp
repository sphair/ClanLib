...
Client::Client()
{
	...
	cc.connect(network_client.sig_event_received(), std::function<void(const NetGameEvent&)>(this, &Client::on_event_received));
	cc.connect(network_client.sig_connected(), std::function<void()>(this, &Client::on_connected));
	cc.connect(network_client.sig_disconnected(), std::function<void()>(this, &Client::on_disconnected));
