...
RadioButton::RadioButton(GUI *gui) : 
	CL_Window(CL_Rect(256 + 256 + 32, 256 + 180 + 32, CL_Size(256, 180)), &gui->get_gui_manager(), CL_GUITopLevelDescription("Radio Button")),
	gui(gui)
{
	set_draggable(true);
	...
	info_selected = new Info(gui, this);
	info_selected->set(xoffset, yoffset, "Selected");
	yoffset += gap;
	info_unselected = new Info(gui, this);
	info_unselected->set(xoffset, yoffset, "Unselected");
	yoffset += gap;
	info_group_selection_changed = new Info(gui, this);
