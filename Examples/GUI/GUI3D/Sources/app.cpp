...
{
	window1->object_matrix = CL_Mat4f::identity();
	...
	window1->object_matrix.multiply(CL_Mat4f::rotate(CL_Angle( value, cl_degrees), 1.0f, 0.0f, 0.0f));

	value = ( (float) slider_1_yrotation->component->get_position() * 360.0f) / (float) slider_1_xrotation->component->get_max();
	window1->object_matrix.multiply(CL_Mat4f::rotate(CL_Angle( value, cl_degrees), 0.0f, 1.0f, 0.0f));

	value = ( (float) slider_1_zrotation->component->get_position() * 360.0f) / (float) slider_1_xrotation->component->get_max();
	window1->object_matrix.multiply(CL_Mat4f::rotate(CL_Angle( value, cl_degrees), 0.0f, 0.0f, 1.0f));
}
