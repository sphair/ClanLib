...
Options::Options(CL_GUIManager &gui, CL_Rect gui_position) : CL_GUIComponent(&gui, CL_GUITopLevelDescription("Options", gui_position, false))
{
	...
	checkbox_ypos += checkbox_gap + 8;
	checkbox_alphatest = create_checkbox(checkbox_xpos, checkbox_ypos, "Use AlphaTest on Balls", is_alphatest_set);
	checkbox_alphatest->func_state_changed().set(this, &Options::checkbox_alphatest_changed);
	checkbox_ypos += checkbox_gap + 8;
...
void Options::checkbox_alphatest_changed()
{
	is_alphatest_set = checkbox_alphatest->is_checked();
}
