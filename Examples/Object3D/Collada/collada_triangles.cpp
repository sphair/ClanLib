...
void CL_Collada_Triangles_Impl::load_inputs(CL_DomElement &polylist_element, std::vector<CL_Collada_Source> &sources, CL_Collada_Vertices &vertices)
{
	...
	{
			if(cur_child.get_node_name() == cl_text("input"))
			{
...
// "position_offset" - Offset into positions list (usually 0) (Used when writing to the output)
// "vertices_offset" - Offset into vertices list (usually 0) (Used when writing to the output)
