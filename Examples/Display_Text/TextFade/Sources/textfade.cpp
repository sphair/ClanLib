...
int TextFade::start(const std::vector<std::string> &args)
{
	...
	Slot slot_quit = window.sig_window_close().connect(this, &TextFade::on_window_close);

	// Connect a keyboard handler to on_key_up()
	Slot slot_input_up = (window.get_ic().get_keyboard()).sig_key_up().connect(this, &TextFade::on_input_up);

	// Get the graphic context
	GraphicContext gc = window.get_gc();

	Image background = Image(gc, "../../Game/DiceWar/Resources/lobby_background2.png");
	...
	Font standard_font(gc, font_description);

	ResourceManager app_resources("../Font/Resources/resources.xml");
	Font_Sprite sprite_font(gc, "ClanFont", &app_resources);
...
void TextFade::draw_text(GraphicContext &gc, Font &font, int ypos, const char *text)
{
	// For this example, to keep things simple, we only handle ASCII characters

	int gc_width = gc.get_width();

	Size text_size = font.get_text_size(gc, text);
	...
	while(*text)
	{
		...
		if (alpha > 0.0f)
			font.draw_text(gc, xpos, ypos,  buffer, Colorf(0.4f, 0.4f, 1.0f, alpha));
...
// A key was pressed
void TextFade::on_input_up(const InputEvent &key)
{
